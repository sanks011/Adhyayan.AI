...
const CustomNode = ({ data, id }: NodeProps) => {
  ...
  const getBorderColor = () => {
    if (nodeData.isSelected) return "border-orange-500 border-2 shadow-lg shadow-orange-500/50"
    if (nodeData.isRoot) return "border-blue-500 border-2"
    return nodeData.isRead ? "border-green-500" : "border-gray-600"
  }

  // Determine background color
  const getBackgroundColor = () => {
    if (nodeData.isRoot) return "bg-gradient-to-br from-blue-900 to-blue-800"
    if (nodeData.isSelected) return "bg-gradient-to-br from-orange-900 to-orange-800"
    return nodeData.isRead
  ...
  const getTextColor = () => {
    if (nodeData.isRoot) return "text-blue-100"
    if (nodeData.isSelected) return "text-orange-100"
    return nodeData.isRead ? "text-green-100" : "text-gray-100"
  }
  ...
  return (
    <div className="relative">
      ...
      {data.hasChildren && (
        <div
          className="absolute -left-8 top-1/2 transform -translate-y-1/2 w-6 h-6 rounded-full bg-gray-700 hover:bg-gray-600 flex items-center justify-center cursor-pointer border border-gray-500 transition-all duration-200 z-10"
      ...
      <div
        className={cn(
          "px-6 py-4 min-w-40 max-w-60 rounded-lg flex items-center justify-center border cursor-pointer transition-all duration-200 hover:scale-105",
          getBorderColor(),
          getBackgroundColor(),
          nodeData.isRoot ? "font-bold text-lg" : "font-medium text-sm",
        )}
      ...
      >
        ...
        {!nodeData.isRoot && (
          <div
            className={cn(
              "absolute -left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 rounded-full flex items-center justify-center cursor-pointer transition-all duration-200 border-2 border-gray-800",
              nodeData.isRead ? "bg-green-500 hover:bg-green-400" : "bg-gray-600 hover:bg-gray-500",
            )}
            onClick={(e) => {
              e.stopPropagation()
              toggleReadStatus()
          ...
          >
            {nodeData.isRead && <div className="h-2 w-2 bg-white rounded-full"></div>}
          </div>
        )}

        <div className={cn("text-center leading-tight", getTextColor())}>{nodeData.label}</div>
      </div>
      ...
      <Handle
        ...
        className="w-2 h-2 border-2 border-gray-400 bg-gray-600"
        style={{ left: -8 }}
      />
      <Handle
        type="source"
...
function MindMapContent({ mindMapId }: { mindMapId: string }) {
  ...
  const loadMindMapData = async () => {
    try {
      ...
      if (response.success && response.mindMap) {
        ...
        console.log("Loaded mind map data:", mindMap.mindmap_data)
  ...
  const initialNodes = useMemo(() => {
    if (!mindMapData?.nodes) {
      console.log("No mind map nodes available")
      return []
    }

    console.log("Processing mind map nodes:", mindMapData.nodes)
    ...
    mindMapData.nodes.forEach((node: any, index: number) => {
      ...
      const position = node.position || {
        x: index === 0 ? 400 : 200 + index * 150,
        y: index === 0 ? 300 : 200 + index * 100,
      }

      console.log(`Creating node ${node.id} at position:`, position)
  ...
  const initialEdges = useMemo(() => {
    if (!mindMapData?.edges) {
      console.log("No mind map edges available")
      return []
    }

    console.log("Processing mind map edges:", mindMapData.edges)
  ...
  useEffect(() => {
    console.log("Updating nodes with new data")
    ...
    <div className="h-screen bg-black flex">
      ...
      <div className="flex-1 h-full">
        <div className="h-full flex flex-col">
          {/* Header */}
          <div className="p-6 border-b border-neutral-700 bg-gray-900/50">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <IconFocus className="h-4 w-4 mr-2" />
                  Fit View
                </Button>
                <div className="text-sm text-gray-400">{nodes.filter((n) => !n.hidden).length} nodes visible</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1" ref={reactFlowWrapper}>
            <ReactFlow
              ...
              maxZoom={2}
            >
              <Controls className="bg-neutral-800 text-white border-neutral-700" showInteractive={false} />
